...
export function renderRelationships(container: HTMLElement, model: ParsedModel): void {
  ...
  const linkSel = svg.append('g').selectAll<SVGLineElement, GraphLink>('line')
    .data(links)
    .enter()
    .append('line')
  ...
  const linkLabelSel = svg.append('g').selectAll<SVGTextElement, GraphLink>('text')
    .data(links)
    .enter()
    .append('text')
    .attr('font-size', '9px')
  ...
  const nodeSel = svg.append('g').selectAll<SVGGElement, GraphNode>('g')
    .data(nodes)
    .enter()
    .append('g')
  ...
  const zoom = d3.zoom<SVGSVGElement, unknown>()
    .scaleExtent([0.2, 4])
    .on('zoom', e => {
      svg.selectAll<SVGGElement, unknown>('g').attr('transform', e.transform.toString())
    })
