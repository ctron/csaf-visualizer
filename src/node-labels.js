...
export function labelAnchor(isLeaf, isTopLevel) {
    if (isLeaf)
        return { x: NODE_GAP, y: 4, anchor: 'start' };
    if (isTopLevel)
        return { x: -NODE_GAP, y: 4, anchor: 'end' };
    return { x: NODE_GAP, y: 18, anchor: 'start' };
}
export function renderNodeLabels(el, opts) {
    const { name, nameColor, nameFontSize = '12px', nameFontWeight = '400', nameTitle, productId, extraLines = [], pihEntries: entries = [], anchor } = opts;
